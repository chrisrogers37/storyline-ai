...
class TestInstagramPostingWorkflow:
    ...
    async def test_routing_all_posts_go_to_telegram_first(
        ...
        """Test ALL posts go to Telegram for approval first.

        The current design routes all scheduled posts through Telegram
        for human review. Instagram API posting only happens when a user
        explicitly clicks "Auto Post" in the Telegram bot interface.
        """
        from src.services.core.posting import PostingService
        ...
        with patch.object(PostingService, "__init__", lambda x: None):
            ...
            # Even with healthy Instagram service, posts go to Telegram first
            mock_instagram = Mock()
        ...
        # All posts go through Telegram for approval
        assert result["method"] == "telegram_manual"
        assert result["success"] is True
    ...
    async def test_instagram_posting_full_flow(
        ...
        try:
            with patch.object(PostingService, "__init__", lambda x: None):
                ...
                service.lock_service = Mock()
                service.telegram_service = Mock()
                # Mock settings_service to return non-dry-run settings
