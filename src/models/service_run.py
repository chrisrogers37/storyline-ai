...
class ServiceRun(Base):
    ...
    service_name = Column(String(100), nullable=False, index=True)  # 'MediaIngestionService', 'PostingService'
    method_name = Column(String(100), nullable=False)  # 'scan_directory', 'process_pending_posts'

    # Execution context
    user_id = Column(UUID(as_uuid=True), ForeignKey("users.id"))  # Who triggered it (NULL for automated)
    triggered_by = Column(String(50), default="system")  # 'user', 'system', 'scheduler', 'cli'
