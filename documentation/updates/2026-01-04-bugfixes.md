...
> **Historical Note (2026-02-10):** Line numbers and code locations referenced here are from the pre-refactor codebase (v1.0.1-v1.2.0). The TelegramService was refactored in v1.6.0 — command handlers moved to `telegram_commands.py`, callback handlers to `telegram_callbacks.py`, etc. Bugs referenced here have been fixed and the fixes carried forward through the refactor.

---

## 🐛 Bugs Fixed

### 1. Service Run Repository - Wrong Column Name ✅ FIXED
...
**Root Cause**:
Column was renamed in the model (ServiceRun.metadata → ServiceRun.context_metadata) but repository method signature wasn't updated.
...
def create_run(
    ...
    metadata: Optional[dict] = None,  # ❌ Wrong parameter name
) -> str:
    run = ServiceRun(
        ...
        metadata=metadata,  # ❌ Wrong column name
    )
...
def create_run(
    ...
    triggered_by: str = "system",
    input_params: Optional[dict] = None,
    context_metadata: Optional[dict] = None,  # ✅ Correct parameter name
) -> str:
    run = ServiceRun(
        service_name=service_name,
        method_name=method_name,
        ...
        triggered_by=triggered_by,
        input_params=input_params,
        context_metadata=context_metadata,  # ✅ Correct column name
    )
```
...
### 2. Scheduler - Date Mutation Bug ✅ FIXED
...
for day_offset in range(days):
    ...
    for post_num in range(posts_per_day):
        if hour_offset >= 24:
            base_date = base_date + timedelta(days=1)  # ❌ Mutates outer loop variable!
```
...
for post_num in range(posts_per_day):
    ...
    if hour_offset >= 24:
        hour_offset -= 24
        base_date = base_date + timedelta(days=1)  # ❌ Mutation
...
for post_num in range(posts_per_day):
    ...
    post_date = base_date  # ✅ Local variable
    if hour_offset >= 24:
        hour_offset -= 24
        post_date = base_date + timedelta(days=1)  # ✅ No mutation
...
```
Post 1: 2026-01-04 22:00  ✅
Post 2: 2026-01-04 23:20  ✅
Post 3: 2026-01-05 00:40  ✅ (crossed midnight correctly)
Post 4: 2026-01-05 22:00  ✅
Post 5: 2026-01-05 23:20  ✅
Post 6: 2026-01-06 00:40  ✅ (crossed midnight correctly, NOT 2026-01-07)
```

---

### 3. Health Check - Missing SQLAlchemy text() Wrapper ✅ FIXED (Deployment)
...
### 4. Telegram Service - Missing Lock Creation ✅ FIXED (Deployment)
...
**Issue**:
When users clicked "Posted ✅" button, the system created history record and updated media counter, but **did not create 30-day lock** to prevent reposting.
...
# After
self.media_repo.increment_times_posted(str(queue_item.media_item_id))
self.lock_service.create_lock(str(queue_item.media_item_id))  # ✅ Added
self.queue_repo.delete(queue_id)
```
...
|-----|----------|--------|--------|------------|
| Service Run Metadata | High | ✅ Fixed | Observability | 2026-01-04 |
| Scheduler Date Mutation | High | ✅ Fixed | Scheduling | 2026-01-04 |
| Health Check text() | High | ✅ Fixed | Monitoring | 2026-01-04 (Deployment) |
| Lock Creation | **Critical** | ✅ Fixed | Core Feature | 2026-01-04 (Deployment) |
...
- ⚠️ Unit tests require test database configuration fix (postgres user issue)

### Bugs #3 & #4 (Health Check + Lock Creation)
- ✅ Deployed to production
- ✅ Verified with 3 successful posts
- ✅ Database queries confirm correct behavior
- ✅ Test coverage added for lock creation
...
- ✅ Service execution tracking working
- ✅ Scheduler correctly handling midnight-crossing windows
- ✅ Health checks accurate
- ✅ 30-day lock system preventing reposts
